; RUN: split-file %s %t
; RUN: not not opt -passes=cj-ir-verifier < %S/Inputs/counterexample-unknown-dst-reference-slot.ll -disable-output 2>&1 | FileCheck %s -check-prefixes=REVIEW,UNKNOWN,ABORT
; RUN: not not opt -passes=cj-ir-verifier < %S/Inputs/dynamic-addrspacecast-unknown.ll -disable-output 2>&1 | FileCheck %s -check-prefixes=DYNAMIC,UNKNOWN,ABORT
; RUN: not not opt -passes=cj-ir-verifier < %S/Inputs/constant-cast-nonzero-offset-unknown.ll -disable-output 2>&1 | FileCheck %s -check-prefixes=NONZERO,UNKNOWN,ABORT
...
; RUN: not not opt -passes=cj-ir-verifier < %t/select-reference-slots.ll -disable-output 2>&1 | FileCheck %s -check-prefixes=SELECT,UNKNOWN,ABORT

; REVIEW: Bare memcpy/memmove payload provenance is unknown; use cj_array_copy_ref, a typed helper, or supply typed provenance.
; REVIEW-NEXT: call void @llvm.memmove.p1i8.p1i8.i64
...
; NONZERO: Bare memcpy/memmove payload provenance is unknown; use cj_array_copy_ref, a typed helper, or supply typed provenance.
...
; SELECT-NEXT: call void @llvm.memmove.p1i8.p1i8.i64
