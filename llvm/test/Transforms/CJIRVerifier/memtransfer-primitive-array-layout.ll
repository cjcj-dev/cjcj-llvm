...
; RUN: not not opt -passes=cj-ir-verifier < %t/reject-static-array-source.ll -disable-output 2>&1 | FileCheck %s -check-prefixes=STATIC,ABORT
; RUN: not not opt -passes=cj-ir-verifier < %t/reject-untyped-source.ll -disable-output 2>&1 | FileCheck %s -check-prefixes=UNTYPED,ABORT
...
; REF: Bare memcpy/memmove payload provenance is unknown; use cj_array_copy_ref, a typed helper, or supply typed provenance.
...
; STATIC: Bare memcpy/memmove payload provenance is unknown; use cj_array_copy_ref, a typed helper, or supply typed provenance.
