...
; CHECK-NEXT: [[LOAD:%.*]] = load atomic i8 addrspace(1)*, i8 addrspace(1)* addrspace(1)* %slot seq_cst, {{.*}}!cj.colored.value
; CHECK-NEXT: [[BITS:%.*]] = ptrtoint i8 addrspace(1)* [[LOAD]] to i64
; CHECK-NEXT: [[SHIFT:%.*]] = load i64, i64* @g_cjLoadShift
; CHECK-NEXT: [[ADDRESS:%.*]] = lshr i64 [[BITS]], [[SHIFT]]
; CHECK-NEXT: [[RESULT:%.*]] = inttoptr i64 [[ADDRESS]] to i8 addrspace(1)*
; CHECK-NEXT: ret i8 addrspace(1)* [[RESULT]]
  store volatile i64 %word, i64* %alias
  %result = call i8 addrspace(1)* @llvm.cj.atomic.load(i8 addrspace(1)* %heap, i8 addrspace(1)* addrspace(1)* %slot, i32 5)
