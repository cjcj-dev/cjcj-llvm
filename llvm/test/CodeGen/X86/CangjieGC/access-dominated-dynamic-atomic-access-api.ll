...
; CHECK-NEXT: %result = call i8 addrspace(1)* @CJ_MCC_AtomicReadReference(
; CHECK-NEXT: ret i8 addrspace(1)* %result
  store volatile i64 %word, i64* %alias
  %result = call i8 addrspace(1)* @llvm.cj.atomic.load(i8 addrspace(1)* %heap, i8 addrspace(1)* addrspace(1)* %slot, i32 %order)
