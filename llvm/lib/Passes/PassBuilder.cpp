...
Error PassBuilder::parseModulePass(ModulePassManager &MPM,
  ...
  if (startsWithDefaultPipelineAliasPrefix(Name)) {
    ...
    if (CJPipeline) {
      ...
      // Repair the exact dynamically-sized boxed-payload copy emitted by the
      // frontend before CJIRVerifier rejects untyped AS0-to-AS1 memcpy.
      MPM.addPass(CJBoxedValueBarrier());
      // Preserve the concrete CType carried by the exact direct-cj2c return
      // materialization sequence before the verifier sees the opaque call.
      MPM.addPass(CJTypedCallReturnCopy());
      // Promote load/store/dbg-only spill slots (e.g. %this.debug) back to SSA
      // before verification.  mem2reg does not lift typed allocas that still
      // have bitcast/memcpy users, which the verifier families depend on.
      // ZGC inserts barriers on the C2-optimized ideal graph
      // (zBarrierSetC2.cpp ZBarrierSetC2::optimize_loops / late expansion);
      // equivalent here: normalize then verify.
      MPM.addPass(createModuleToFunctionPassAdaptor(PromotePass()));
      // Verify that ir generated by the front end of the cangjie is correct.
      MPM.addPass(CJIRVerifier());
      MPM.addPass(CJTypedReadHelper());
